...
<script>
    $(document).ready(function(){
        ...
        function submitComment(){
            $.ajax({
                url: "submitDiaryReplyServlet",
                type: "POST",
                async:false,
                data: {
                    "toId":toId,
                    "diaryId": ${requestScope.diaryComment.did},
                    "diaryCommentId":${requestScope.diaryComment.dCid},
                ...
                success: function (result) {
                    ...
                    //     "                        </span>\n" +
                    //     "                        <span class=\"ml-2\">"+result.diaryReplyTime+"</span>\n" +
                    //     "                    </div>\n" +
                    //     "                    <div id=\"replyContent\" class=\"mt-3 commentContent\">\n" + result.commentContent +
                    //     "                    </div>\n" +
                    //     "                    <div id=\"replyFooter\" class=\"mt-1\">\n" +
                    //     "                        <button id=\"likeButton\" class=\"btn btn-sm btn-light-green\" type=\"button\" data-toggle=\""+result.diaryReplyId+"\"><i class=\"far fa-thumbs-up\"> " +result.commentLikeCount+"</i></button>\n" +
    ...
    <div id="article" class="float-left mt-3" style="width: 700px">
        ...
        <div id="content1" class="clearfix mt-4">
            ...
            <div id="commentList" class="mt-3">
                <%
                    DiaryComment diaryComment = (DiaryComment) request.getAttribute("diaryComment");
                    UserService userService = new UserServiceImpl();
                    UserInfoService userInfoService = new UserInfoServiceImpl();
                    UserInfo userInfo = userInfoService.findUserInfoById(diaryComment.getUid());
                    User user = userService.findUserByUid(diaryComment.getUid());
                %>
        ...
        <!-- 全部回复 -->
        <%
            List<DiaryReply> diaryReplyList = (List<DiaryReply>) request.getAttribute("diaryReplyList");
        %>
        <div id="content2" class="clearfix mt-4">
            <div >
                <h2 id="replyListTitle" class="contentTitle float-left">全部回复 · · · · · · </h2>
                <button id="commentButton" class="btn btn-sm btn-light-green m-0 ml-5" data-toggle="<%=diaryComment.getUid()%>"><i class="fas fa-comment-dots"></i> 回复楼主</button>
            </div>
            <div id="replyList" class="mt-3">
                ...
                <div id="reply"  class="borderTop mt-2 pt-3">
                    ...
                    <div id="replyFooter" class="mt-1">
                        <button id="likeButton" class="btn btn-sm btn-light-green" type="button" data-toggle="<%=diaryReply.getDRId()%>"><i class="far fa-thumbs-up"> <%=diaryReply.getDRLikeCount()%></i></button>
